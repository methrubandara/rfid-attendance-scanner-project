...
<link rel="icon" type="image/x-icon" href="img/AprilFools.png">
...
<head>
    <title>AMSA Eagle Eye</title>
    <meta name="description" content="Online Attendance System for Advanced Math and Science Academy. Built by AMSA Fullstack Class of 2024.">

    <script>

        //wait for the promise result to be fulfilled before returning
        async function onSignIn(googleUser)
        {

            //temp thing
            $uid = 43003849;
            fetch("imageBuilder.php",{
                method: "POST",
                body: JSON.stringify({
                    data: $uid
                })
            ...
            {
                console.log(r);
                if (r.includes("valid") && (!r.includes("invalid")))
                {
                    window.location.href = 'https://amsaeagleeye.org/class-view';
                    //window.location.href = 'https://amsaeagleeye.org/sessionCheck.php';
                    //aparently cant redirect until alert is closed
                    //alert('redirect');

                }
                else
                {
                    alert('This is not an AMSA teacher email!');
                }
            });
    ...
    <style>
        body {
            background: linear-gradient(-45deg, rgb(63, 63, 63), black);
            background: linear-gradient(-45deg, rgb(63, 63, 63), black);
...
<body>
    <div
        style=" display: flex; width: 100%; height: 100vh; flex-direction: column;  overflow: hidden; justify-content: center; align-items: center;">
        <div style=" display: flex; padding: 3em; ">
            <!--<img src="img/signIn.png" width="400">-->
            <img src="img/AprilFools.png" width="400">
        </div>

        <!--sign in button-->

        <div id="g_id_onload" data-client_id="461340649461-ie2bnohc4sjmjic6gan7gp37o7kgeqiq" data-context="signin"
            data-ux_mode="popup" data-callback="onSignIn" data-itp_support="true" data-auto_select="true" data-use_fedcm_for_prompt="true" >
        </div>
